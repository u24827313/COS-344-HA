...
void Hole18::design(Terrain& terrain) {
    Hole::design(terrain);
    ...
    // ---- MAIN cylinder (center body) ----
    float mainRadius = 3.0f;
    ...
    // ---- LOWER BULGE (south extension) ----
    float lowerRadius = 2.2f;
    ...
    auto addCylinderRocks = [&](const glm::vec3& center, float orbitRadius, int count) {
        for (int i = 0; i < count; ++i) {
            ...
            if (isInsideAnyGreen(rockPos)) continue;
    ...
    addCylinderRocks(mainPos,  mainRadius + 0.2f,  16);
    addCylinderRocks(upperPos, upperRadius + 0.2f, 14);
